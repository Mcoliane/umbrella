    ...
    def handle_command(self, screen, raw: str):
        ...
        if name == "profile":
            if args and args[0] == "set":
                ...
                if not text:
                    self.add_local_event("system", "Usage: /profile set <text> — replaces the profile the mayor sees every turn.")
                    return
                try:
                    out = self.client.set_profile(text)
                except Exception as ex:  # noqa: BLE001
                    self.add_local_event("error", f"Profile update failed: {type(ex).__name__}")
                    return
                if isinstance(out, dict) and out.get("ok"):
                    self.add_local_event("system", "Profile updated — the mayor sees it from the next message on.")
                else:
