...
ACTION_GUIDE = {
    ...
    "skill.files.query": "find, read, and summarize the user's LOCAL files, and (with allowWrite:true) create/edit/reorganize them (scoped to safe folders like Desktop/Documents/Downloads; no delete) — inputs: task (required, e.g. 'find the gateway.gold analysis on my desktop and summarize it', or 'write a summary note to my desktop'), roots (optional folder(s)), allowWrite (optional: set true ONLY for create/edit/move tasks). Use for 'it's on my desktop', 'read/summarize this file', 'what's in this folder', 'write/edit a file', 'reorganize these files' — NOT the code agent.",
