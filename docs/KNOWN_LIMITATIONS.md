...
- There is no automatic promotion/sync from short-term `memory-core` entries to long-term node memory; agents must promote explicitly (queued promotions now drain automatically once enqueued).
