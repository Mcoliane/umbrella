...
P2=$!
python3 "$ROOT/services/policy/app.py" --host 127.0.0.1 --port "$POLICY_PORT" --umbrella-root "$ROOT" --catalog-url "$CATALOG_URL" >"$ROOT/tmp/umbrella04-mgs-policy.out" 2>"$ROOT/tmp/umbrella04-mgs-policy.err" &
P3=$!
python3 "$ROOT/services/plugin_host/app.py" --host 127.0.0.1 --port "$PLUGIN_HOST_PORT" --umbrella-root "$ROOT" --catalog-url "$CATALOG_URL" >"$ROOT/tmp/umbrella04-mgs-host.out" 2>"$ROOT/tmp/umbrella04-mgs-host.err" &
P4=$!
python3 "$ROOT/services/execution/app.py" --host 127.0.0.1 --port "$EXEC_PORT" --umbrella-root "$ROOT" --policy-url "$POLICY_URL" --catalog-url "$CATALOG_URL" --plugin-host-url "$PLUGIN_HOST_URL" >"$ROOT/tmp/umbrella04-mgs-exec.out" 2>"$ROOT/tmp/umbrella04-mgs-exec.err" &
P5=$!
python3 "$ROOT/services/session/app.py" --host 127.0.0.1 --port "$SESSION_PORT" --umbrella-root "$ROOT" --catalog-url "$CATALOG_URL" --execution-url "$EXEC_URL" >"$ROOT/tmp/umbrella04-mgs-session.out" 2>"$ROOT/tmp/umbrella04-mgs-session.err" &
P6=$!
...
ns = post(mem_url + '/v1/namespaces', {'id': 'team', 'owner_type': 'system', 'owner_id': 'test'})
assert ns.get('id') == 'team', ns
node = post(mem_url + '/v1/nodes', {
    'node_id': 'fact:team:alpha',
    'namespace': 'team',
    'kind': 'fact',
    'title': 'Alpha Fact',
    'content': {'value': {'summary': 'alpha node payload'}},
})
assert node.get('node_id') == 'fact:team:alpha', node
...
memory_get_item = entries.get('memory.get.skill')
assert memory_get_item is not None, catalog_items
assert any(action.get('id') == 'skill.memory.get' for action in memory_get_item.get('actions', [])), memory_get_item
...
registered_worker = post(policy_url + '/v1/policy/agents/register', {'agentId': 'programming-agent', 'source': 'external', 'capabilities': ['knowledge.read']})
assert registered_worker.get('ok') is True, registered_worker

profile = post(session_url + '/v1/shop-profiles', {
    'profileId': 'memory-get-development-profile',
    'name': 'Memory Get Development Profile',
    'shopType': 'business',
    'defaultTitle': 'Programming Agent',
    'defaultShopName': 'Development Shop',
    'enabledActionIds': ['skill.memory.get'],
...
assert any(a.get('id') == 'skill.memory.get' for a in session.get('availableActions', [])), session
...
worker = post(session_url + f'/v1/sessions/{session_id}/originations', {
    ...
    'shopProfileId': 'memory-get-development-profile',
})
assert worker.get('shop', {}).get('enabledActionIds') == ['skill.memory.get'], worker
...
assert plugin_result.get('summary') == 'memory.get returned fact:team:alpha', invoked

print('memory get skill PASS')
PY

echo "umbrella0.4 memory get skill contract PASS"
